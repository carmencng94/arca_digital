### Día 20 de febrero de 2026: Corrección y mejora de vista de detalle
- Se corrigió el fallo donde el modal de detalle no mostraba los datos al pulsar "Ver detalle" en la tarjeta de animal.
- Se implementó un modal profesional para la vista detallada, ubicado fuera del contenedor principal para evitar conflictos de CSS y garantizar superposición visual.
- Se añadieron los campos `descripcion`, `medicacion` y `castrado` a la base de datos y modelo de Animal, asegurando su gestión en el backend y frontend.
- Se actualizaron los estilos para el modal de detalle, asegurando un z-index alto y diseño profesional.
- Se documentó el funcionamiento y la estructura de datos ampliada en la documentación técnica.
